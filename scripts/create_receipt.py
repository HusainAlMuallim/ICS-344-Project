...
def lambda_handler(event, context):
    ...
    # FIX 1: Validate orderId to prevent path traversal in filename
    if not re.match(r'^[a-zA-Z0-9\-]+$', orderId):
        return {"status": "err", "msg": "invalid orderId"}
        ...
        if status < 120:
            ...
            if conn is None:
                ...
                for k in got_items.keys():
                    ...
                    # BEFORE: "SELECT ... WHERE itemId = " + item_id
                    # AFTER:  parameterized query with ? placeholder
                    res = cur.execute(
                        "SELECT itemId, name, price FROM inventory WHERE itemId = ?",
                        (item_id,)
                    )
                    row = res.fetchone()
                    if row is None:
                        continue
                    item_id, price, name = row
            ...
            response = order_table.update_item(
                ...
                ExpressionAttributeValues={':orderStatus': 200}
            )
